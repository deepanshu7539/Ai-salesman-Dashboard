...
const CsvUploader: React.FC<CsvUploaderProps> = ({ onFilesUpload }) => {
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (validFiles.length) {
      setCsvFiles(prevFiles => [...prevFiles, ...validFiles]);
      onFilesUpload([...csvFiles, ...validFiles]);
  ...
  const handleRemoveFile = (index: number) => {
    ...
    onFilesUpload(csvFiles.filter((_, i) => i !== index)); // Update the parent component with the remaining files
  };
